...
// registered, creates a new row with 201 otherwise. Making register
// safe to call every session is the "user-friendly" half of the
// registration gate — clients don't need to probe-then-create.
app.post(
  '/api/v1/projects',
...
// Hard-delete a project only when no data references it. We check
// every child table; if any has rows, we refuse with a breakdown so
// the caller knows what to clean up (or can archive instead).
